...
<div class="section1">
  ...
  <div class="table">
    ...
    <script type="text/html" id="barDemo">
      ...
      {{# if(!d.reply_to_id){ }}
      <button class="layui-btn layui-btn-xs" lay-event="view">查看被回复</button>
      {{# } }}
    </script>
  </div>
</div>
...
<script>
  ...
  layui.use(['element', 'layer', 'util'], function () {
    ...
    // 拿到传递过来的值
    var source_id = window.location.href.split("?")[1]
    ...
    table.render({
      elem: '#comments'
      , url: BaseUrl+'/api/comment/get_list?like=0'
      , headers: {
        'x-auth-token': token
      }
      ...
      , cols: [[
        ...
        , { field: 'img', width: '15%', title: '头像',templet: function (d) {return "<div><img src=" + fullUrl(BaseUrl,d.img) + "></div>"} }
        , { field: 'user_id', width: '15%', title: '评论人' , templet:
    ...
    table.on('tool(comments)', function (obj) {
      var data = obj.data;
      if (obj.event === 'detail') {
        window.location.href = "./view_comments_detail.jsp?" + data.comment_id;//参数名称为index
      } else if (obj.event === 'view') {
        window.location.href = "./comments_table.jsp?" + data.comment_id;//参数名称为index
      }
    });
