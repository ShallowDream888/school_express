            ...
            <div class="layui-row">
                <form class="layui-form" action="">
                    <div class="layui-upload">
                        <button type="button" class="layui-btn" id="runner">上传头像</button>
                        <div class="layui-upload-list">
                            <img class="layui-upload-img" id="runner_img">
                            <p id="demoText"></p>
                        </div>
...
<script>
    var BaseUrl = baseUrl()
    let runner_id = location.search.substring(1)
    layui.use(['upload', 'element', 'layer', 'laydate', 'layedit'], function () {
        ...
        // 提交按钮校验权限
        // if (   user_group == "管理员" ||$check_action('/runner/view', 'add') || $check_action('/runner/view', 'set')) {
        ...
        var uploadInst = upload.render({
            elem: '#runner'
            , url: BaseUrl + '/api/runner/upload?' //此处用的是第三方的 http 请求演示，实际使用时改成您自己的上传接口即可。
            , headers: {
                'x-auth-token': token
            }, before: function (obj) {
                //预读本地文件示例，不支持ie8
                obj.preview(function (index, file, result) {
                    $('#runner_img').attr('src', fullUrl(BaseUrl, result)); //图片链接（base64）
                });

                element.progress('runner', '0%'); //进度条复位
            ...
            , progress: function (n, elem, e) {
                element.progress('runner', n + '%'); //可配合 layui 进度条元素使用
                if (n == 100) {
        ...
        if (runner_id !== '') {
            ...
            async function axios_get_4() {
                ...
                for (let key in form_data) {
                    if (key == 'avatar') {
                        runner_img.src = BaseUrl + form_data.avatar
                    }
                }
                for (let key in form_data) {
                    if (key == 'email_state') {
            ...
            async function axios_get_5() {
                const {data: rese} = await axios.get(BaseUrl + '/api/runner/get_obj', {
                    params: {
                        user_id: runner_id
                    }
                })
        ...
        submit.onclick = async function () {
            ...
            if (!username.value) {
                ...
                layer.msg('密码不能为空');
            } else if (!runner_id && (password.value.length > 16 || password.value.length < 5)) {
                ...
                form_sub.student_number = student_number.value
                //文本
                if (user_group === '管理员' || (form_sub['runner_id'] && $check_field('set', 'student_number')) || (!form_sub['runner_id'] && $check_field('add', 'student_number'))) {
                } else {
                ...
                form_sub.full_name = full_name.value
                //文本
                if (user_group === '管理员' || (form_sub['runner_id'] && $check_field('set', 'full_name')) || (!form_sub['runner_id'] && $check_field('add', 'full_name'))) {
                } else {
                    $("#full_name").attr("disabled", true);
                    $("#full_name > input[name='file']").attr('disabled', true);
                }
                if (user_group === '管理员' || (form_sub['runner_id'] && $check_field('set', 'gender')) || (!form_sub['runner_id'] && $check_field('add', 'gender'))) {
                } else {
                ...
                form_sub.major = major.value
                //文本
                if (user_group === '管理员' || (form_sub['runner_id'] && $check_field('set', 'major')) || (!form_sub['runner_id'] && $check_field('add', 'major'))) {
                } else {
                ...
                form_sub.department = department.value
                //文本
                if (user_group === '管理员' || (form_sub['runner_id'] && $check_field('set', 'department')) || (!form_sub['runner_id'] && $check_field('add', 'department'))) {
                } else {
                ...
                form_sub.dormitory = dormitory.value
                //文本
                if (user_group === '管理员' || (form_sub['runner_id'] && $check_field('set', 'dormitory')) || (!form_sub['runner_id'] && $check_field('add', 'dormitory'))) {
                } else {
                ...
                if (runner_id == '') {
                    console.log("新增/Add")
                    const {data: ress} = await axios.post(BaseUrl + '/api/runner/add?', form_sub, {
                        headers: {
                            'x-auth-token': token,
                            'Content-Type': 'application/json'
                        }
                    ...
                    const {data: res_data2} = await axios.post(BaseUrl + '/api/runner/set?runner_id=' + form_sub.runner_id, form_sub, {
                        headers: {
                            'x-auth-token': token,
                            'Content-Type': 'application/json'
                        }
