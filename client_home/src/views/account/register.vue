<template>
  <div id="account_register" class="page_account">
    <div class="warp">
      <div class="container">
        <div class="row">
              ...
              <div class="form_register">
                <b-form>
                  <b-form-group id="input-group-1" label="账号:" label-for="input-1">
                    <b-form-input id="input-1" v-model="form.username" placeholder="账号" trim
                                  type="text"></b-form-input>
                  </b-form-group>

                  <b-form-group id="input-group-2" label="设置密码:" label-for="input-2">
                    <b-form-input id="input-2" v-model="form.password" placeholder="5个字符到16个字符"
                                  trim type="password"></b-form-input>
                  </b-form-group>

                  <b-form-group id="input-group-3" label="确认密码:" label-for="input-3">
                    <b-form-input id="input-3" v-model="confirm_password" placeholder="确认密码"
                                  trim type="password"></b-form-input>
                  </b-form-group>

                  <b-form-group id="input-group-4" label="昵称:" label-for="input-4">
                    <b-form-input id="input-4" v-model="form.nickname" placeholder="2个字符到12个字符" trim
                                  type="text"></b-form-input>
                  </b-form-group>

                  <b-form-group id="input-group-5" label="邮箱:" label-for="input-5">
                    <b-form-input id="input-5" v-model="form.email" placeholder="例: example@123.com"
                                  trim type="email"></b-form-input>
                  </b-form-group>

                  <b-form-group id="input-group-6" label="手机号:" label-for="input-6">
                    <b-form-input id="input-6" v-model="form.phone" placeholder="例: 18955552312"
                                  trim type="text"></b-form-input>
                  </b-form-group>
                  ...
                  <!--雇主用户-->
                  <form_employer_user v-if="form.user_group === '雇主用户'" :form="form_sub"></form_employer_user>
                  <!--跑腿用户-->
                  <form_runner v-if="form.user_group === '跑腿用户'" :form="form_sub"></form_runner>
...
/* 雇主用户 */
import form_employer_user from "@/components/diy/form_employer_user";
/* 跑腿用户 */
import form_runner from "@/components/diy/form_runner";
      ...
      if (!ret && user_group) {
        if (user_group === "雇主用户") {
          ...
          if (!ret && !form_sub["student_number"]) {
            ...
            res = await this.$get("~/api/employer_user/count?", {"student_number": form_sub["student_number"]});
            if (res.result) {
              ret = "学号已存在!";
            }
          }
        ...
        if (user_group === "跑腿用户") {
          ...
          if (!ret && !form_sub["student_number"]) {
            ...
            res = await this.$get("~/api/runner/count?", {"student_number": form_sub["student_number"]});
            if (res.result) {
              ret = "学号已存在!";
            }
          }
