<template>
  <div id="root_demo" class="page_root">
    <div class="warp">
      <div class="container">
        <div class="row">
          <div class="col">
          ...
          <div class="col-12">
            <div class="sw_no">
              ...
              <div class="card_notice">
                <div class="notice_title">公告栏
                  <router-link to="notice/list">更多</router-link>
                </div>
                <!-- 公告组件 -->
        ...
        <div class="row">
          <div class="col-12">
            ...
            <div class="article_recommend">
              ...
              <div class="more_box">
                <router-link class="more" to="/article/list">
                  <span>更多</span>
                </router-link>
              </div>
            ...
            <div v-else class="overflow-auto">
              <table id="list_article" :aria-colcount="fields_article.length" aria-busy="false"
                ...
                <tr v-for="(o, i) in list_table_article" :key="i">
                  <td v-for="(oj,n) in fields_goods" @click="to_details('goods',o)">
                    <img v-if="oj.type && oj.type == '图片' " v-default-img="'/img/default.png'" :src="$fullUrl(o[oj.key])"
                         alt="">
    ...
    <!-- 推荐代领任务模块(开始) -->
    <div v-if="$check_action('/collecting_tasks_on_behalf/list', 'get')" class="warp">
      <!-- 容器 -->
      <div class="container">
        <div class="row">
          <div class="col-12">
            ...
            <div class="diy_recommend">
              ...
              <div class="more_box">
                <router-link class="more" to="/collecting_tasks_on_behalf/list">
                  <span>更多</span>
                </router-link>
              </div>
              <div class="switch" @click="show_list_collecting_tasks_on_behalf = !show_list_collecting_tasks_on_behalf">
                <div class="switch_box">
                  <span v-if="show_list_collecting_tasks_on_behalf"> 切换表格 </span>
                  <span v-else> 切换列表 </span>
                </div>
              </div>
            </div>
            <!-- 代领任务推荐列表组件 -->
            <list_collecting_tasks_on_behalf v-if="show_list_collecting_tasks_on_behalf"
                                             :list="list_collecting_tasks_on_behalf"/>
            <div v-else class="overflow-auto">
              <table id="list_diy" :aria-colcount="fields_collecting_tasks_on_behalf.length" aria-busy="false"
                     class="table b-table table-striped table-hover"
                     role="table">
                <thead>
                <tr>
                  <th v-for="(o,i) in fields_collecting_tasks_on_behalf" :key="i">
                    {{ o.label }}
                  </th>
                </tr>
                </thead>
                <tbody>
                <tr v-for="(o, i) in list_table_collecting_tasks_on_behalf" :key="i"
                    @click="to_details('collecting_tasks_on_behalf',o,'collecting_tasks_on_behalf_id')">
                  <td v-for="(oj,n) in fields_collecting_tasks_on_behalf">
                    <img v-if="oj.type && oj.type == '图片' " v-default-img="'/img/default.png'" :src="$fullUrl(o[oj.key])"
                         alt="">
...
export default {
  ...
  data() {
    return {
      ...
      show_list_collecting_tasks_on_behalf: true,
      list_collecting_tasks_on_behalf: [],
      vm_arr_collecting_tasks_on_behalf: [
        "collecting_tasks_on_behalf_id",
        "task_number",
        "task_name",
        "publishing_users",
        "item_picture",
      ...
      fields_collecting_tasks_on_behalf: [
        {
          key: "task_number",
          label: "任务编号",
          type: "文本"
  ...
  methods: {
    ...
    // 获取代领任务列表
    get_collecting_tasks_on_behalf() {
      let url = "~/api/collecting_tasks_on_behalf/get_list?";
      this.$get(url, {
        "page": 1,
        "size": 8
      }, (json) => {
        if (json.result) {
          this.list_collecting_tasks_on_behalf = json.result.list;
        }
      })
    },
  ...
  mounted() {
    this.get_collecting_tasks_on_behalf();
    this.get_menu();
    this.get_slides();
    this.get_article();
    // this.get_link();
  ...
  computed: {
    list_table_picking_order() {
      var list = this.list_collecting_tasks_on_behalf;
      var list_table = [];
      for (let i = 0; i < list.length; i++) {
        list_table[i] = {};
        this.vm_arr_collecting_tasks_on_behalf.map((o) => {
          // 第二个中括号是对象的属性
