<template>
  <div>
    <b-form-group v-if="$check_register_field('add','student_number','/runner/view')" id="input-group-${idx}"
                  label="学号" label-for="input-${idx}">
      <b-form-input id="input-${idx}" v-model="form['student_number']" placeholder="学号" trim
                    type="text"></b-form-input>
    </b-form-group>
    <b-form-group v-if="$check_register_field('add','full_name','/runner/view')" id="input-group-${idx}" label="姓名"
                  label-for="input-${idx}">
      <b-form-input id="input-${idx}" v-model="form['full_name']" placeholder="姓名" trim type="text"></b-form-input>
    </b-form-group>
    <b-form-group v-if="$check_register_field('add','gender','/runner/view')" id="input-group-${idx}" label="性别"
                  label-for="input-${idx}">
      <b-form-select id="input-${idx}" v-model="form['gender']" :options="getDropList(list_gender)"></b-form-select>
    </b-form-group>
    <b-form-group v-if="$check_register_field('add','major','/runner/view')" id="input-group-${idx}" label="专业"
                  label-for="input-${idx}">
      <b-form-input id="input-${idx}" v-model="form['major']" placeholder="专业" trim type="text"></b-form-input>
    </b-form-group>
    <b-form-group v-if="$check_register_field('add','department','/runner/view')" id="input-group-${idx}" label="院系"
                  label-for="input-${idx}">
      <b-form-input id="input-${idx}" v-model="form['department']" placeholder="院系" trim type="text"></b-form-input>
    </b-form-group>
    <b-form-group v-if="$check_register_field('add','dormitory','/runner/view')" id="input-group-${idx}" label="宿舍"
                  label-for="input-${idx}">
